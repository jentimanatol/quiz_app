...
class QuizApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        lm_check.pack(side="left", padx=12)
        ...
        for i in range(4):
            ...
            if self.default_fg is None:
                # capture the default foreground color once
        ...
        self.legend_lbl = ttk.Label(self, textvariable=self.legend_var, padding=(10, 0, 10, 10), foreground="#006400")
    ...
    def update_option_colors(self):
        # Apply learning-mode coloring logic
        ...
        for i, letter in enumerate(LETTER_CHOICES):
            if letter == correct_letter:
                self.rb_widgets[i].configure(fg="green")
        # If wrong selected, color that choice red
        if selected and selected != correct_letter:
            try:
                idx = LETTER_CHOICES.index(selected)
                self.rb_widgets[idx].configure(fg="red")
            except ValueError:
                pass  # ignore if unexpected
    ...
    def load_question(self, index: int):
        ...
        # Legend text depending on learning mode
        if self.learning_mode.get():
            self.legend_var.set("Learning Mode: Correct = green, Your wrong choice = red")
        else:
            self.legend_var.set("")
    ...
    def on_toggle_learning_mode(self):
        # Update legend and recolor options
        if self.learning_mode.get():
            self.legend_var.set("Learning Mode: Correct = green, Your wrong choice = red")
        else:
            self.legend_var.set("")
